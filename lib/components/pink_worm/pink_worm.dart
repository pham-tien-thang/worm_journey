...
import 'poison_reverse_cloud.dart';
...
class PinkWorm extends Worm {
  ...
  double get speedMoveIntervalScale => 0.65;
  double get snailMoveIntervalScale => 2.0;
  double get poisonSlowMoveIntervalScale => 1.5;
  ...
  Future<void> onLoad() async {
    await super.onLoad();
    add(MagnetRadarComponent(segmentSize: config.segmentSize, priority: 5));
    add(DizzyStarsComponent(segmentSize: config.segmentSize, priority: 20));
    add(AntidoteBurstComponent(segmentSize: config.segmentSize, priority: 20));
    add(BombExplosionComponent(segmentSize: config.segmentSize, priority: 20));
    add(FreezeBurstComponent(segmentSize: config.segmentSize, priority: 20));
    add(
      PoisonReverseCloudComponent(
  ...
  void onItemEffectAdded(String itemId) {
    if (itemId == ItemType.coconut.effectTypeId) {
      stats.currentHardness = stats.originalBaseHardness + 10;
      setHasHelmet(true);
      _effectBlinkAccumulator = 0;
      _effectBlinkShow = true;
    }
    if (itemId == ItemType.speed.effectTypeId ||
        itemId == ItemType.snail.effectTypeId) {
  ...
  void onItemEffectRemoved(String itemId) {
    ...
    if (itemId == ItemType.speed.effectTypeId ||
        itemId == ItemType.snail.effectTypeId ||
        itemId == ItemType.dizzy.effectTypeId) {
      _syncMoveInterval();
    }
  }

  /// Cập nhật moveInterval theo speed/snail còn đang bật; không có thì về base.
  void _applySpeedSnailMoveInterval(String itemId) {
    if (itemId == ItemType.speed.effectTypeId) {
  ...
  void _syncMoveInterval() {
    final base = config.moveInterval;
    if (isPoisonedReverse && hasItemEffect(ItemType.dizzy.effectTypeId)) {
      setMoveInterval(base * poisonSlowMoveIntervalScale);
    } else if (hasItemEffect(ItemType.speed.effectTypeId)) {
      setMoveInterval(base * speedMoveIntervalScale);
    } else if (hasItemEffect(ItemType.snail.effectTypeId)) {
      setMoveInterval(base * snailMoveIntervalScale);
    } else {
      setMoveInterval(base);
    }
  }

  void applyPoisonReverse() {
    setPoisonedReverse(true);
    _syncMoveInterval();
  }
  ...
  void update(double dt) {
    super.update(dt);
    if (_antidoteBurstRemaining > 0)
      _antidoteBurstRemaining = (_antidoteBurstRemaining - dt).clamp(0.0, 1.0);
    if (_bombExplosionRemaining > 0)
      _bombExplosionRemaining = (_bombExplosionRemaining - dt).clamp(0.0, 1.0);
    if (_freezeBurstRemaining > 0)
      _freezeBurstRemaining = (_freezeBurstRemaining - dt).clamp(0.0, 1.0);
    ...
    if (hasItemEffect(ItemType.freeze.effectTypeId) && gameTime != null) {
      ...
      setFreezeEndBlink(
        freezeTimeLeft != null && freezeTimeLeft > 0 && freezeTimeLeft <= 1.0,
      );
    } else {
      setFreezeEndBlink(false);
    }

    if (!hasItemEffect(ItemType.coconut.effectTypeId) || gameTime == null)
      return;
    final list =
        itemEffects
            .where((e) => e.itemId == ItemType.coconut.effectTypeId)
            .toList();
