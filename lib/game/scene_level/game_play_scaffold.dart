...
class _GamePlayScaffoldState extends State<GamePlayScaffold> {
  ...
  void _onBlockedItemTap(BuildContext scaffoldContext) {
    ...
    ScaffoldMessenger.of(
      scaffoldContext,
    ).showSnackBar(SnackBar(content: Text(appL10n.itemBlockedInLevel)));
  }
  ...
  Future<bool> _onBuyItem(ItemModel item, BuildContext scaffoldContext) async {
    ...
    if (!ok) {
      if (mounted && scaffoldContext.mounted) {
        ScaffoldMessenger.of(
          scaffoldContext,
        ).showSnackBar(SnackBar(content: Text(appL10n.notEnoughCoins)));
      }
      return false;
    }
    final q = _itemQuantities[item.effectTypeId] ?? 0;
    ...
    return Stack(
      ...
      children: [
        ...
        Positioned(
          ...
          right: -2,
          child: Text(
            appL10n.quantityShort(quantity),
            style: const TextStyle(
              fontSize: 12,
              color: Colors.black,
              fontWeight: FontWeight.bold,
